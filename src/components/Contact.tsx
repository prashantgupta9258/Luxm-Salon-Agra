...
import { collection, addDoc, serverTimestamp, getDoc, doc, query, where, getDocs, onSnapshot } from 'firebase/firestore';
...
export default function Contact() {
  ...
  useEffect(() => {
    const unsub = onSnapshot(doc(db, 'settings', 'salon'), (docSnap) => {
      if (docSnap.exists()) {
        ...
        DAYS.forEach((_, i) => {
          defaultHours[i] = { isOpen: i !== 0 && i !== 1, open: i === 6 ? '09:00' : '10:00', close: i === 6 ? '18:00' : '20:00' };
        });
  ...
  useEffect(() => {
    ...
    if (!daySettings || !daySettings.isOpen) {
      setClosedMessage(`Our salon is closed on ${dateObj.toLocaleDateString('en-US', { weekday: 'long' })}s.`);
      return;
    }

    const dateClosures = settings.closedPeriods?.filter((p: any) => p.date === formData.date);
    if (dateClosures && dateClosures.length > 0) {
      const wholeDay = dateClosures.find((p: any) => !p.startTime && !p.endTime);
      if (wholeDay) {
        setClosedMessage(`Our salon is closed on this date${wholeDay.reason ? `: ${wholeDay.reason}` : '.'}`);
      } else {
        const partials = dateClosures.filter((p: any) => p.startTime && p.endTime);
  ...
  useEffect(() => {
    if (formData.date) {
      ...
      getDocs(q).then(snapshot => {
        ...
        const appts = JSON.parse(localStorage.getItem('luxm_appointments') || '[]');
        setBookedSlots(appts.filter((a: any) => a.date === formData.date).map((a: any) => a.time));
      });
    } else {
      setBookedSlots([]);
    }
  ...
  const getAvailableSlots = () => {
    ...
    const closedWholeDay = settings.closedPeriods?.find((p: any) => p.date === formData.date && !p.startTime && !p.endTime);
    if (closedWholeDay) {
      return [];
    }
    ...
    for (let m = openMinutes; m < closeMinutes; m += 30) {
      ...
      const isPeriodClosed = settings.closedPeriods?.find((p: any) => p.date === formData.date && p.startTime && p.endTime && p.startTime <= slotTimeStr && p.endTime > slotTimeStr);
      ...
      slots.push({ label: timeLabel, value: timeLabel, isBooked: bookedSlots.includes(timeLabel) });
    }
    return slots;
  };
              ...
              <form className="space-y-6" onSubmit={handleSubmit}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="flex flex-col gap-2">
                    ...
                    <input type="text" id="name" required value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="border-b border-gray-300 py-2 focus:outline-none focus:border-[#C4A47C] bg-transparent transition-colors text-gray-900 placeholder-gray-400" placeholder="Jane Doe" />
                  </div>
                  <div className="flex flex-col gap-2">
                    <label htmlFor="phone" className="text-sm font-medium text-gray-700">Phone Number *</label>
                    <input type="tel" id="phone" required value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} className="border-b border-gray-300 py-2 focus:outline-none focus:border-[#C4A47C] bg-transparent transition-colors text-gray-900 placeholder-gray-400" placeholder="(555) 000-0000" />
                  </div>
                </div>
                
                <div className="flex flex-col gap-2">
                  <label htmlFor="email" className="text-sm font-medium text-gray-700">Email Address</label>
                  <input type="email" id="email" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} className="border-b border-gray-300 py-2 focus:outline-none focus:border-[#C4A47C] bg-transparent transition-colors text-gray-900 placeholder-gray-400" placeholder="jane@example.com" />
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="flex flex-col gap-2">
                    ...
                    />
                    {closedMessage && <span className={`text-xs mt-1 ${getAvailableSlots().length === 0 ? 'text-red-500' : 'text-orange-500'}`}>{closedMessage}</span>}
                  </div>
                  <div className="flex flex-col gap-2">
                    ...
                    <select 
                      ...
                      disabled={!formData.date || getAvailableSlots().length === 0}
                    >
                      <option value="" disabled>{formData.date ? "Select a time" : "Select a date first"}</option>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <select id="service" required value={formData.service} onChange={e => setFormData({...formData, service: e.target.value})} className="border-b border-gray-300 py-2 focus:outline-none focus:border-[#C4A47C] bg-transparent transition-colors text-gray-900">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <textarea id="message" rows={3} value={formData.notes} onChange={e => setFormData({...formData, notes: e.target.value})} className="border-b border-gray-300 py-2 focus:outline-none focus:border-[#C4A47C] bg-transparent transition-colors resize-none text-gray-900 placeholder-gray-400" placeholder="Specific stylist requests, condition of hair..."></textarea>
                </div>
                ...
                <div className="flex items-start gap-4 pt-4 border-t border-gray-300/50 mt-4">
                  ...
                  <div className="text-sm space-y-1 w-full max-w-[200px]">
                    {settings && settings.businessHours ? (
                      ['Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday', 'Monday'].map((dayName, idx) => {
                        ...
                        return (
                          <div key={dayName} className={`flex justify-between ${!daySetting.isOpen ? 'text-gray-500' : ''}`}>
                      ...
                      <>
                        <p className="flex justify-between"><span>Tue - Fri</span> <span>10am - 8pm</span></p>
                        <p className="flex justify-between"><span>Sat</span> <span>9am - 6pm</span></p>
                        <p className="flex justify-between text-gray-500"><span>Sun - Mon</span> <span>Closed</span></p>
                      </>
                    )}
                  </div>
                </div>
